...
const ltaBillUploadDir = "./Documents/LTABill";

if (!fs.existsSync(ltaBillUploadDir)) {
  fs.mkdirSync(ltaBillUploadDir, { recursive: true });
}

const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    cb(null, ltaBillUploadDir);
  },
  filename: (req, file, cb) => {
    const ext = path.extname(file.originalname);
    const fieldIndex = file.fieldname.match(/\[(\d+)\]/)?.[1] || 'unknown';
    const fieldBase = file.fieldname.split('[')[0];
    const filename = `IT_${req.params.employeeId}_${fieldBase}_${fieldIndex}_${Date.now()}_${Math.random().toString(36).substring(2, 9)}${ext}`;
    cb(null, filename);
  }
});
...
const  LtaBillFileUpload = (req, res, next) => {
  upload(req, res, (err) => {
    if (err) {
...
export default LtaBillFileUpload;