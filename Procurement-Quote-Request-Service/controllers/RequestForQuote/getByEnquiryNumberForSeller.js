...
async function getByEnquiryNumberForSeller(req, res) {
  try {
    ...
    let sellerProfile;
    const sellerResponse = await getSellerByDealingProducts(sellerId);
    ...
    sellerProfile = sellerResponse.data;
    ...
    const categoriesResponse = await getCategoriesByIds(categoryIds);
    if (!categoriesResponse.hasError) {
      categories = categoriesResponse.data || [];
    }

    const subCategoriesResponse = await getSubCategoriesByIds(subCategoryIds);
    if (!subCategoriesResponse.hasError) {
