...
async function create(req, res) {
  ...
  try {
    ...
    // see what happens the logic to find last order number is correct but the probleam is that if in previous api response
    // when this api runs at that time for example two entries at time stored and the order number is ORD/2025-26/0001 and ORD/2025-26/0002
    // but both have same createdAt like 2025-05-15T14:29:11.724+00:00

    // so next time when this api run at that time it finds last orderNumber like ORD/2025-26/0001 and it stores the orderNumber like
    // ORD/2025-26/0002 but this orderNumber is already in previous response still it it counts 1 as last

    // so the probleam is that if in firsttime if there is two entry the order Number is ORD/2025-26/0001 and ORD/2025-26/0002
    // then next response if two entries then it must store ORD/2025-26/0003 and ORD/2025-26/0004
    // but currently it stores order Number is ORD/2025-26/0001 and ORD/2025-26/0002 for first response and
    // second time it stores ORD/2025-26/0002 and ORD/2025-26/0003
    // so tell me what to do because i thing you need to chnage this logic to find last order

    const lastOrder = await OrderDetailsFromSeller.findOne({
