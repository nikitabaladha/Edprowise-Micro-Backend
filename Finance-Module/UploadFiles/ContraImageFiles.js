...
const chequeFileForContraDir = "./Documents/FinanceModule/ChequeImageForContra";
const chequeImageForContraDir = "./Images/FinanceModule/ChequeImageForContra";

[chequeFileForContraDir, chequeImageForContraDir].forEach((dir) => {
...
const FinanceFileUpload = multer({
  storage: multer.diskStorage({
    destination: (req, file, cb) => {
      ...
      if (file.fieldname === "chequeImageForContra") {
        cb(null, isPdf ? chequeFileForContraDir : chequeImageForContraDir);
      } else {
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (["chequeImageForContra"].includes(file.fieldname)) {
...
}).fields([{ name: "chequeImageForContra", maxCount: 1 }]);
