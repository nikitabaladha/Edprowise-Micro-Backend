...
async function processBatchRefundForDate(
  ...
  try {
    ...
    // Group refunds by paymentMode, date, AND feeType for receipt creation
    const refundsByGroup = {};

    refundsData.forEach((refund) => {
      const key = `${refund.paymentMode}_${refund.refundDate}_${refund.feeType}`;
    ...
    // Process each group (payment mode + date + feeType combination)
    for (const [key, group] of Object.entries(refundsByGroup)) {
      const { paymentMode, refundDate, feeType, totalRefundAmount, refunds } =
        group;
