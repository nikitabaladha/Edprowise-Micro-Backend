...
async function deleteBySchoolAndYear(req, res) {
  try {
    ...
    // 🔸 Perform bulk delete
    const result = await BSPLLedger.deleteMany({ schoolId, academicYear });
    ...
    return res.status(200).json({
      hasError: false,
      message: `Successfully deleted ${result.deletedCount} BSPLLedger(s) for academic year ${academicYear} for School ${schoolId}.`,
    });
  } catch (error) {
