  ...
  try {
    ...
    if (!id) {
      ...
      return res.status(400).json({
        hasError: true,
        message: "Receipt Entry ID is required.",
      });
    }
    ...
    // Find the existing receipt entry
    const existingReceiptEntry = await ReceiptEntry.findOne({
      _id: id,
      schoolId,
      financialYear,
    }).session(session);
    ...
    const updatedReceiptEntry = await ReceiptEntry.findOneAndUpdate(
      {
        _id: id,
        schoolId,
      },
    ...
    return res.status(200).json({
      ...
      data: updatedReceiptEntry,
    });
  } catch (error) {
    await session.abortTransaction();
    session.endSession();

    console.error("Error updating Receipt Entry approval status:", error);
    ...
    if (error.name === "CastError") {
      return res.status(400).json({
        hasError: true,
        message: "Invalid Receipt Entry ID.",
      });
    }
