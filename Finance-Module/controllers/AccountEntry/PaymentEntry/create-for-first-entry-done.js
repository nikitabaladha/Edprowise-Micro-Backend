...
// Helper function to get or create opening balance record
async function getOrCreateOpeningBalanceRecord(
  ...
  // Find the most recent balance record before the current entry date
  ...
  if (previousRecord) {
    ...
    // DON'T use balanceType from previousRecord, use the one from the ledger
  } else if (ledger) {
    ...
    if (TDSorTCS && TDSTCSRateWithAmountBeforeGST > 0) {
      ...
      if (!tdsTcsLedger) {
        ...
        await updateOpeningClosingBalance(
          ...
          newPaymentEntry._id,
          0, // debit amount
          TDSTCSRateWithAmountBeforeGST // credit amount
        );
      } else {
        // Add debug logging to see what ledger is found
        console.log("Found TDS/TCS Ledger:", {
