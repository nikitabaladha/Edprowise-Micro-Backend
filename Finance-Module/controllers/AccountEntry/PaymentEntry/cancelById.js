...
async function recalculateAllAffectedLedgers(
  ...
  // 1. Add item ledgers (Credit entries in payments)
  ...
  // 2. Add payment mode ledger (Debit entry in payments)
...
async function cancelById(req, res) {
  ...
  try {
    ...
    const entryDate = existingPayment.entryDate;
    const itemDetails = existingPayment.itemDetails;
    const TDSorTCS = existingPayment.TDSorTCS;
    const TDSTCSRateWithAmountBeforeGST =
      existingPayment.TDSTCSRateWithAmountBeforeGST || 0;
    const subTotalAmountAfterGST = existingPayment.subTotalAmountAfterGST || 0;
    const subTotalOfCredit = existingPayment.subTotalOfCredit || 0;

    const ledgerIdWithPaymentMode = existingPayment.ledgerIdWithPaymentMode;
