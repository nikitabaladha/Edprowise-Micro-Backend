...
async function getAllCashBookBySchoolId(req, res) {
  try {
    ...
    for (const entry of paymentEntries) {
      ...
      const hasCashInItems = itemsWithLedgerNames.some(
        (item) => item.groupLedgerName === "Cash"
      );

      if (
        hasCashInItems ||
        groupLedgerWithPaymentMode?.groupLedgerName === "Cash"
      ) {
    ...
    for (const entry of contraEntries) {
      ...
      const hasCashInItems = itemsWithLedgerNames.some(
        (item) =>
          item.groupLedgerName === "Cash" ||
          item.groupLedgerNameOfCashAccount === "Cash"
      );
...
export default getAllCashBookBySchoolId;
