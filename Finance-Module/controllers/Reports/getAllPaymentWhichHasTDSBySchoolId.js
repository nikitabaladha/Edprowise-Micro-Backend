...
async function getAllPaymentWhichHasTDSBySchoolId(req, res) {
  try {
    ...
    const query = {
      ...
      TDSorTCS: "TDS",
      entryDate: {
        $gte: start.toDate(), // Greater than or equal to start date
        $lte: end.toDate(), // Less than or equal to end date
      },
    };
    ...
    return res.status(200).json({
      hasError: false,
      message:
        "TDS payment entries fetched successfully with date range filter.",
      data: formattedData,
    });
  } catch (error) {
    console.error("Error fetching TDS payment entries:", error);
...
export default getAllPaymentWhichHasTDSBySchoolId;
