...
export async function getQuoteProposalBySellerId(sellerId, options = {}) {
  try {
    const response = await axios.get(
      `${process.env.PROCUREMENT_QUOTE_PROPOSAL_SERVICE_URL}/api/quote-proposal-by-seller-id/${sellerId}`,
      {
        params: options,
      }
    );
