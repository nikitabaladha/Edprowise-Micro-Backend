...
const createAndUpdateTemplate = async (req, res) => {
  try {
    ...
    if (sellerTemplate) {
      ...
      sellerTemplate = new SellerRegistrationEmailTemplate({
        mailFrom: mailFrom,
        subject,
        content,
      });
    ...
    res.status(200).json({
      hasError: false,
      message: "Seller Registration template saved successfully!",
    });
  } catch (error) {
    res.status(500).json({
      hasError: true,
      message: "Seller Registration Email template Not saved",
    });
  }
};
