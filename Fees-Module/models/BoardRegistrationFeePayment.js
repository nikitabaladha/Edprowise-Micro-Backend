...
const BoardRegistrationFeeCounter = mongoose.model(
  "BoardRegistrationFeeCounter",
  BoardRegistrationFeeCounterSchema
);
...
BoardRegistrationFeePaymentSchema.pre("save", async function (next) {
  ...
  while (attempts > 0) {
    try {
      const counter = await BoardRegistrationFeeCounter.findOne({
        schoolId: this.schoolId,
      });

      if (!this.receiptNumberBrf) {
        const updatedCounter =
          await BoardRegistrationFeeCounter.findOneAndUpdate(
            { schoolId: this.schoolId },
            { $inc: { receiptSeq: 1 } },
            { new: true, upsert: true }
          );
        this.receiptNumberBrf = `BRF/${updatedCounter.receiptSeq
          .toString()
          .padStart(6, "0")}`;
      }
      ...
      if (this.paymentMode === "Online" && !this.transactionId) {
        ...
        this.transactionId = `TXN-${Date.now()
          .toString(36)
          .toUpperCase()}-${updatedCounter.transactionSeq
          .toString()
          .padStart(4, "0")}`;
      }
      ...
      if (
        err.code === 11000 &&
        (err.message.includes("receiptNumberBrf") ||
          err.message.includes("transactionId"))
      ) {
        attempts--;
...
BoardRegistrationFeePaymentSchema.pre(
  "findOneAndUpdate",
  async function (next) {
    const update = this.getUpdate();
    const newStatus = update.$set?.status;
