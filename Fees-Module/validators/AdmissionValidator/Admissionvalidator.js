...
export const AdmissionValidator = {
  validate: (data) => {
    const schema = Joi.object({
      ...
      masterDefineClass: Joi.string().required().custom((value, helpers) => {
        if (!isValidObjectId(value)) {
          return helpers.message("Class ID must be a valid ObjectId.");
        }
        return value;
      }).messages({
        "any.required": "Class is required."
      }),
      section: Joi.string().required().custom((value, helpers) => {
        if (!isValidObjectId(value)) {
          return helpers.message("Section ID must be a valid ObjectId.");
        }
        return value;
      }).messages({
        "any.required": "Section is required."
      }),
      masterDefineShift: Joi.string().required().custom((value, helpers) => {
        if (!isValidObjectId(value)) {
          return helpers.message("Shift ID must be a valid ObjectId.");
        }
        return value;
      }).messages({
        "any.required": "Shift is required."
      ...
      previousSchoolResult: Joi.string().allow(null, ""),
      tcCertificate: Joi.string().allow(null, ""),
      proofOfResidence: Joi.string().optional(),
      aadharPassportNumber: Joi.string().required().custom((value, helpers) => {
        const aadharPattern = /^\d{12}$/;
        const passportPattern = /^[A-Z]\d{7}$/;
        if (!aadharPattern.test(value) && !passportPattern.test(value)) {
      ...
      aadharPassportFile: Joi.string().optional(),
      studentCategory: Joi.string().valid("General", "OBC", "ST", "SC").required().messages({
        "any.required": "Student category is required.",
        "any.only": "Student category must be one of General, OBC, ST, or SC."
      }),
