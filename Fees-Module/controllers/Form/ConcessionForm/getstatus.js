...
const getConcessionStatus = async (req, res) => {
  ...
  try {
    const student = await ConcessionForm.findOne({ _id: id, schoolId }).select(
    ...
    res.status(200).json({
      hasError: false,
      message: "Student Concession status retrieved successfully.",
      student: {
    ...
    res.status(500).json({
      hasError: true,
      message: err.message,
      details: "Error retrieving student concession status.",
    });
  }
};

export default getConcessionStatus;
