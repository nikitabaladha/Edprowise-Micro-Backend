import StudentTCfrom from "../../../models/TCForm.js";
...
const getTCStatus = async (req, res) => {
  ...
  try {
    const student = await StudentTCfrom.findOne({ _id: id, schoolId }).select(
    ...
    res.status(200).json({
      hasError: false,
      message: "Student tc status retrieved successfully.",
      student: {
    ...
    res.status(500).json({
      hasError: true,
      message: err.message,
      details: "Error retrieving student tc status.",
    });
  }
};

export default getTCStatus;
