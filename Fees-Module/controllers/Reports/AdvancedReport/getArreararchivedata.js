...
export const getArrearFeesArchive = async (req, res) => {
  try {
    ...
    if (parseInt(endYear) - parseInt(startYear) !== 1) {
      return res.status(400).json({
        message:
          "Invalid academic year range. End year must be one year after start year",
      });
    }
    ...
    if (!Array.isArray(archiveData.defaulters)) {
      console.error("Defaulters is not an array:", {
        schoolId,
        academicYear,
        defaulters: archiveData.defaulters,
      });
    ...
    const responseData = {
      ...
      defaulters: archiveData.defaulters.map((defaulter) => ({
        ...
        installments: Array.isArray(defaulter.installments)
          ? defaulter.installments.map((installment) => ({
              paymentDate: installment.paymentDate || "-",
              cancelledDate: installment.cancelledDate || null,
              reportStatus: Array.isArray(installment.reportStatus)
              ...
              feeTypes:
                installment.feeTypes &&
                (installment.feeTypes instanceof Map ||
                  typeof installment.feeTypes === "object")
                  ? Object.fromEntries(
                      installment.feeTypes instanceof Map
    ...
    const classOptions = [
      ...new Set(
        responseData.defaulters
          .map((defaulter) => defaulter.className)
          .filter((cls) => cls)
      ),
    ]
      .sort()
    ...
    const sectionOptions = [
      ...new Set(
        responseData.defaulters
          .map((defaulter) => defaulter.sectionName)
    ...
    const installmentOptions = [
      ...new Set(
        responseData.defaulters.flatMap((defaulter) =>
          Array.isArray(defaulter.installments)
            ? defaulter.installments
                .map((installment) => installment.installmentName)
                .filter((name) => name)
            : []
        )
      ),
    ...
    const paymentModeOptions = [
      ...new Set(
        responseData.defaulters.flatMap((defaulter) =>
          Array.isArray(defaulter.installments)
            ? defaulter.installments
                .map((installment) => installment.paymentMode)
