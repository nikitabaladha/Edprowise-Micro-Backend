...
export const StudentWiseFeesDue = async (req, res) => {
  try {
        ...
        .flatMap((payment) =>
          payment.installments.map((inst) => ({
            ...
            totalCancelledPaidAmount: inst.feeItems.reduce(
              (sum, feeItem) => sum + (feeItem.cancelledPaidAmount || 0),
              0
            ),
          }))
        )
      ...
      for (const instName of allInstallments) {
        ...
        for (const entry of allEntries) {
          ...
          installments.push({
            ...
            cancelledByFeeType:
              entry.type === "Paid"
                ? entry.feeItems.reduce((acc, feeItem) => {
                    const feeTypeName =
                      feeTypeMap[feeItem.feeTypeId?.toString()];
      ...
      if (installments.length > 0) {
        ...
        for (const instName of uniqueInstallments) {
          ...
          totalConcession += instEntries.reduce(
            (sum, inst) => sum + (inst.concession || 0),
            0
          );
    ...
    const paymentModeOptions = Array.from(
      new Set(
        (await SchoolFees.find({ schoolId, academicYear }).lean()).map(
          (fee) => fee.paymentMode
        )
      )
    )
