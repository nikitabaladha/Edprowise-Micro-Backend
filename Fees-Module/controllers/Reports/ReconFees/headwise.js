...
export const getReconFeesHeadwise = async (req, res) => {
  try {
    ...
    const classDataList = await ClassAndSection.find({
      schoolId,
      academicYear,
    }).lean();
    if (!classDataList.length) {
      return res.status(404).json({
        message: "No class data found for the specified academic year",
      });
    }
    ...
    const oneTimeFeesData = await OneTimeFees.find({
      schoolId,
      academicYear,
    }).lean();
    const feesTypesData = await FeesType.find({
      schoolId,
      academicYear,
    }).lean();
    ...
    for (const cls of classDataList) {
      ...
      for (const section of cls.sections) {
        ...
        const classAdmissions = admissionDataList.filter((admission) => {
          const history = admission.academicHistory.find(
            (entry) => entry.academicYear === academicYear
          );
          return (
            history &&
            history.masterDefineClass.toString() === classId &&
            history.section.toString() === sectionId
          );
        });

        const existingStudents = classAdmissions.filter((admission) => {
          const history = admission.academicHistory;
          const currentYearEntry = history.find(
            (entry) => entry.academicYear === academicYear
          );
          const previousYears = history.filter(
            (entry) => entry.academicYear < academicYear
          );
        ...
        const yearlyFeesStructure = feesStructures.find(
          (fs) =>
        ...
        const filteredInstallments = yearlyFeesStructure
          ? yearlyFeesStructure.installments
              .filter((inst) => {
              ...
              .map((inst) => ({
                ...
                isFuture: inst.dueDate
                  ? new Date(inst.dueDate) > currentDate
                  : false,
                fees: inst.fees.map((fee) => {
                  const feeTypeId = fee.feesTypeId?._id?.toString
                    ...
                    : fee.feesTypeId;
                  const feeTypeName =
                    fee.feesTypeId?.feesTypeName || "Unknown Fee";
        ...
        const schoolFees = filteredInstallments.reduce(
          (sum, inst) =>
            sum + inst.fees.reduce((feeSum, fee) => feeSum + fee.amount, 0),
          0
        );

        const oneTimeFees = oneTimeFeesData.find(
          (otf) =>
        ...
        const admFees = oneTimeFees
          ? oneTimeFees.oneTimeFees
              .filter((fee) => {
                const feeType = feesTypesData.find(
                  (type) => type._id.toString() === fee.feesTypeId.toString()
                );
    ...
    res.status(200).json({
      ...
      message:
        "Recon fees headwise data fetched successfully with installment filtering",
    });
  } catch (error) {
