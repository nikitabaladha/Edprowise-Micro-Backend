...
export const getFeesStructureReport = async (req, res) => {
  try {
    ...
    const schoolFeesData = feesStructures.flatMap((structure) => {
      const className =
        classMap[structure.classId.toString()] || structure.classId.toString();
      return structure.installments.flatMap((installment) =>
        structure.sectionIds.flatMap((sectionId) => {
          const sectionName =
            sectionMap[sectionId.toString()] || sectionId.toString();
          return installment.fees.map((fee) => ({
            ...
            feeTypeName:
              feeTypeMap[fee.feesTypeId.toString()] ||
    ...
    const oneTimeFeesData = oneTimeFees.flatMap((otf) => {
      const className =
        classMap[otf.classId.toString()] || otf.classId.toString();
      return otf.sectionIds.flatMap((sectionId) => {
        ...
        return otf.oneTimeFees.map((fee) => ({
          ...
          installment: "-",
          feeTypeName:
            feeTypeMap[fee.feesTypeId.toString()] || fee.feesTypeId.toString(),
          amount: fee.amount || 0,
        }));
      });
    });
    ...
    if (!result.length) {
      return res.status(404).json({
        message: "No fee structure data found for the given academic year",
      });
    }
