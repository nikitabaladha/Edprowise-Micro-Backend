...
const updateBoardExamFeePaymentStatus = async (req, res) => {
  ...
  try {
    const paymentRecord = await BoardExamFeePayment.findOne({
    ...
    const updatedPayment = await BoardExamFeePayment.findOneAndUpdate(
    ...
    res.status(200).json({
      hasError: false,
      message: `Board exam fee payment status updated to ${status} successfully.`,
...
export default updateBoardExamFeePaymentStatus;
