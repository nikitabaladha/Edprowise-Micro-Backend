...
import createFeesStructureValidator from "../../../validators/FeesStructure.js";
  ...
  try {
    ...
    if (!schoolId) {
      return res.status(401).json({
        hasError: true,
        message:
          "Access denied: You do not have permission to update Fees Structure.",
      });
    }
    ...
    const { classId, sectionIds, feesTypeId, academicYear, installments } =
      value;
    ...
    if (duplicate) {
      return res.status(409).json({
        hasError: true,
        message:
          "A fees structure already exists for this class section and and fee type.",
      });
    }
